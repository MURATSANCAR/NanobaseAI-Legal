  ...
  return (
    <div className="app-shell">
      ...
      <aside className={sidebarOpen ? "sidebar open" : "sidebar"}>
        ...
        {canAnalyze && <button className={screen === "pilot-quality" ? "nav active" : "nav"}
          onClick={() => { setScreen("pilot-quality"); setSidebarOpen(false); }}>
          <FlaskConical /><span>Pilot kalite merkezi</span>
        </button>}
        <p className="nav-label">GÜVENLİK</p>
        <button className="nav" onClick={() => notify(`Roller: ${roles.join(", ")}`)}>
          <ShieldCheck /><span>Yetkilerim</span>
        </button>
        {canOperate && <button className={screen === "operations" ? "nav active" : "nav"}
          onClick={() => { setScreen("operations"); setSidebarOpen(false); }}>
          <ServerCog /><span>Production kontrolü</span>
        </button>}
...
function Sprint7Workspace({ token, project, canConfigure, canWrite, onProblem, onNotify }: {
  token: string;
  ...
  return <>
    <div className="title-row">
      <div><p className="eyebrow">SPRINT 7 · DİNAMİK OPERASYON</p>
        <h1>Workflow ve karar merkezi</h1>
        <p>{project
          ? `${project.projectCode} · ${project.name}`
          : "Proje özelindeki merkezler için önce bir proje açın."}</p></div>
...
function OperationsCenter({ token, projects, documents, onProblem }: {
  token: string;
  projects: TenderProject[];
  ...
  return <>
    <div className="title-row">
      <div><p className="eyebrow">YETKİLİ OPERASYON ALANI</p>
        <h1>Production kontrol merkezi</h1>
        <p>Servis, güvenlik, recovery, AI kalite ve pilot kanıtlarını tek yerde izleyin.</p>
      </div>
      <button className="secondary" onClick={() => void load()}><RefreshCw />Yenile</button>
    </div>
    <div className="workspace-tabs operations-tabs">
...
type Sprint9Tab = "quality" | "errors" | "improvements" | "release";

function Sprint9ControlCenter({ token, canOperate, onProblem, onNotify }: {
  token: string;
  canOperate: boolean;
  onProblem: (error: unknown) => void;
  onNotify: (message: string) => void;
  ...
  return <>
    <div className="title-row sprint9-title">
      <div><p className="eyebrow">SPRINT 9 · v1.0 RELEASE CANDIDATE</p>
        <h1>Pilot kalite ve release merkezi</h1>
        <p>Feedback’i kök nedenine ayırın, değişiklikleri deneyle kanıtlayın ve
          go-live kararını fail-closed yönetin.</p>
      </div>
    ...
    <div className="workspace-tabs sprint9-tabs">
      <button className={tab === "quality" ? "active" : ""}
...
function readUnknown(value: Record<string, unknown>, ...keys: string[]) {
