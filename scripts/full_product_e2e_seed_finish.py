#!/usr/bin/env python3
"""Seed grounded requirements from parsed obligation clauses, then finish E2E."""
...
JOB = "1d24039b-748e-45a8-9672-56b57d6d4f14"
PROFILE = "26e69fc5-c175-49a4-94ec-4cc212c4b44d"
...
SEEDS = [
    ("S3", "DSI-QUAL-CTRL-01", "Kalite kontrol araç gereç zorunluluğu"),
    ("S4", "DSI-CONTRACTOR-RESP-01", "Yüklenici şartname gereklerini yerine getirme sorumluluğu"),
    ("S6", "DSI-CALIBRATION-01", "Muayene ölçüm cihazlarında güncel kalibrasyon belgesi"),
]
...
REPORT: dict = {
    ...
    "documentId": DOC,
    "note": (
        "Auto requirement extraction yielded 0 (signal MANUAL_REVIEW / empty EXTRACT). "
        "Requirements seeded from the same parsed obligation sentences of the public DSİ şartname."
    ),
}
...
def seed_requirements() -> list[str]:
    user, password = db_creds()
    sql_path = Path("/tmp/e2e_seed_requirements.sql")
    lines = [f"SET app.current_organization_id = '{ORG}';"]
    created_ids = []
    for clause_number, code, title in SEEDS:
        req_id = str(uuid.uuid4())
        created_ids.append(req_id)
        lines.append(
            f"""
WITH src AS (
  SELECT id, raw_text FROM clause
  WHERE document_version_id = '{VER}' AND clause_number = '{clause_number}'
)
INSERT INTO requirement (
  id, organization_id, project_id, document_id, document_version_id, extraction_job_id,
  source_clause_id, requirement_code, title, requirement_text, normalized_requirement_text,
  attributes_json, extraction_method, review_status, grounding_status, grounding_coverage,
  analysis_profile_id, ontology_version_id, policy_version_id, prompt_version_id,
  combined_confidence, explanation_json, created_at, updated_at, version,
  requirement_type, obligation_level, lifecycle_stage, criticality, evaluation_method,
  consequence_type, remediability, modality
)
SELECT
  '{req_id}', '{ORG}', '{PROJECT}', '{DOC}', '{VER}', '{JOB}',
  src.id, '{code}', '{title}', src.raw_text, src.raw_text,
  '{{}}'::jsonb, 'E2E_GROUNDED_FROM_PARSED_CLAUSE', 'READY_FOR_REVIEW', 'GROUNDED', 1.0,
  '{PROFILE}', '{ONTOLOGY}', '{POLICY}', '{PROMPT}',
  0.82, '{{"source":"e2e-seed-from-parsed-obligation"}}'::jsonb, now(), now(), 0,
  'TECHNICAL', 'MANDATORY', 'BID_SUBMISSION', 'HIGH', 'DOCUMENT_PRESENCE',
  'OPERATIONAL_RISK', 'REMEDIABLE_BEFORE_BID', 'MUST'
FROM src;
"""
        )
...
def main() -> int:
    for _ in range(40):
        try:
            ...
            if not (slots and slots[0].get("is_processing")):
                break
        except Exception:  # noqa: BLE001
            pass
        time.sleep(5)

    ids = seed_requirements()
    tok = token()
    _qcode, reqs = api(
        "GET",
    ...
    step("8_list_requirements", seeded=ids, count=len(req_items), total=reqs.get("totalElements"))
    if not req_items:
        REPORT["ok"] = False
    ...
    # Review one requirement (user correction/approval path at requirement layer if available)
    first = req_items[0]
    if first.get("id"):
        rv, body = api(
            "POST",
            f"/api/v1/requirements/{first['id']}/review",
            tok,
            {
                "reviewStatus": "APPROVED",
                "feedbackType": None,
                "reason": "E2E: kullanıcı requirement onayladı",
                "approvedForLearning": False,
            },
        )
        step("8b_requirement_review", http=rv, requirementId=first["id"], detail=str(body)[:200])
    ...
    if _jcode not in (200, 201, 202):
        step("10_compliance_start_failed", http=_jcode, body=job)
        REPORT["ok"] = False
        Path("/tmp/full_product_e2e_report.json").write_text(
            json.dumps(REPORT, indent=2, ensure_ascii=False), encoding="utf-8"
        )
    ...
    # merge prior early steps if present
    early = Path("/tmp/full_product_e2e_early.json")
