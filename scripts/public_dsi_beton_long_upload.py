#!/usr/bin/env python3
"""Upload DSİ Beton long PDF via portal and collect DI evidence."""
...
PDF = Path("/tmp/long-dsi-beton.pdf")
OUT = Path("/tmp/dsi-beton-long")
...
def main() -> None:
    ...
    code, project = api(
        ...
        tok,
        {
            "name": f"PUBLIC-DSI-BETON-LONG-{stamp}",
            "institutionName": "Devlet Su İşleri Genel Müdürlüğü",
            "tenderRegistrationNumber": f"DSI-BETON-R02-{int(time.time())}",
            "tenderType": "YAPIM",
            ...
            "description": "Public DSİ Beton İşleri Teknik Şartnamesi R02_20241008 long-doc gate",
            "currency": "TRY",
        },
    )
    ...
    doc = json.loads(
        subprocess.check_output(
            [
                ...
                "-F",
                "logicalName=Beton-Isleri-Teknik-Sartnamesi-R02-20241008.pdf",
                "-F",
    ...
    while time.time() - t0 < 7200:
        ...
        msg = (job.get("message") or last.get("message") or "")[:160]
        print(
            f"poll t={int(time.time()-t0)}s status={status} job={job.get('status')} "
