#!/usr/bin/env python3
"""Blind public HBYS E2E RUN_2B — post-fix full pipeline after RUN_2A parser READY.

Produces /tmp/nanobase-hbys-public-e2e/run-2b-post-fix/* and run-2b-review-bundle/*.
Parser routing/batching/checkpoints may differ from RUN_1; prompts/policies/evidence frozen.
"""
...
RUN_DIR = ROOT / "run-2b-post-fix"
BUNDLE = ROOT / "run-2b-review-bundle"
...
SUMMARY: dict[str, Any] = {
    "gate": "PUBLIC-HBYS-BLIND-E2E-RUN2B",
    "runType": "POST_FIX_FULL_E2E",
...
def poll_doc(tok: str, doc_id: str, max_wait: int = 45000) -> dict:
    start = time.time()
    last: dict = {}
...
def main() -> int:
    ...
    if pdf_sha != EXPECTED_SHA:
        fail(f"source hash mismatch {pdf_sha}", "BLOCKED_SOURCE_HASH")
        write_json(RUN_DIR / "run-summary.json", SUMMARY)
        return 2
    SUMMARY["source"] = {
        "officialPage": (
        ...
        "pageCountExpected": EXPECTED_PAGES,
        "file": f"PDF document, version 1.4, {EXPECTED_PAGES} page(s) (pdfinfo/pypdf; file(1) falsely said 20)",
    }
    write_json(RUN_DIR / "02-source-metadata.json", SUMMARY["source"])

    # Runtime already frozen externally; keep pointer
    if (RUN_DIR / "runtime-manifest.json").is_file():
        SUMMARY["runtimeManifest"] = str(RUN_DIR / "runtime-manifest.json")
    ...
    write_json(RUN_DIR / "parser-result.json", parser_result)
    if not page_count or int(page_count) != EXPECTED_PAGES:
        fail(f"pageCount={page_count} expected={EXPECTED_PAGES}", "PARSER")
    if layout_count <= 0:
        fail("layoutBlockCount<=0", "PARSER")
    if ready.get("status") not in DOC_READY:
        fail(f"parser terminal={ready.get('status')}", "PARSER")
    ...
    write_json(RUN_DIR / "clauses.json", clauses_payload)
    ...
    write_json(RUN_DIR / "requirements.json", requirements_payload)
    ...
    write_json(RUN_DIR / "compliance-matrix.json", {
    ...
    write_json(RUN_DIR / "missing-evidence.json", {
        "count": len(missing),
        "items": missing,
    })
    write_json(RUN_DIR / "non-compliant.json", {
    ...
    write_json(RUN_DIR / "review-smoke.json", review_smoke)
    ...
    write_json(RUN_DIR / "ambiguities.json", {"count": len(amb_export), "items": amb_export})
    ...
    write_json(RUN_DIR / "conflicts.json", {"count": len(conflict_export), "items": conflict_export})
    ...
    write_json(RUN_DIR / "hidden-cost-drivers.json", {"count": len(hidden), "items": hidden})
    ...
    write_json(RUN_DIR / "clarification-questions.json", {"count": len(clarifications), "items": clarifications})
    ...
    write_json(RUN_DIR / "company-evidence-review-excerpts.json", {"count": len(excerpts), "items": excerpts})
    ...
    if report_status not in {"REPORT_JOB_COMPLETED", "COMPLETED"}:
        ...
        http, rsize = proxy_download(
            tok, f"/api/v1/report-artifacts/{report_artifact_id}/download", RUN_DIR / "final-report.pdf"
        )
        step("11b_report_proxy", http=http, bytes=rsize)
        if http == 200 and rsize >= 1000 and (RUN_DIR / "final-report.pdf").read_bytes()[:4] == b"%PDF":
    ...
    write_json(RUN_DIR / "deterministic-quality-checks.json", {
    ...
    write_json(RUN_DIR / "audit-summary.json", {
        "http": acode,
    ...
    # Build RUN_2B review bundle (required names from handover)
    diag_parser = DIAG / "parser-result.json"
    diag_caps = DIAG / "page-capabilities.json"
    diag_root = DIAG / "root-cause.json"
    mapping = {
        "01-source-hbys-technical-specification.pdf": PDF_PATH,
        "02-source-metadata.json": RUN_DIR / "02-source-metadata.json",
        "03-run-summary.json": RUN_DIR / "run-summary.json",
        "04-parser-diagnostic-summary.json": (
            diag_parser if diag_parser.is_file() else RUN_DIR / "parser-result.json"
        ),
        "05-page-capabilities.json": diag_caps,
        "06-clauses.json": RUN_DIR / "clauses.json",
        "07-requirements.json": RUN_DIR / "requirements.json",
        "08-compliance-matrix.json": RUN_DIR / "compliance-matrix.json",
        "09-missing-evidence.json": RUN_DIR / "missing-evidence.json",
        "10-non-compliant.json": RUN_DIR / "non-compliant.json",
        "11-risks.json": RUN_DIR / "risks.json",
        "12-ambiguities.json": RUN_DIR / "ambiguities.json",
        "13-conflicts.json": RUN_DIR / "conflicts.json",
        "14-hidden-cost-drivers.json": RUN_DIR / "hidden-cost-drivers.json",
        "15-clarification-questions.json": RUN_DIR / "clarification-questions.json",
        "16-company-evidence-review-excerpts.json": RUN_DIR / "company-evidence-review-excerpts.json",
        "17-deterministic-quality-checks.json": RUN_DIR / "deterministic-quality-checks.json",
        "18-performance.json": RUN_DIR / "performance.json",
        "19-final-report.pdf": RUN_DIR / "final-report.pdf",
        "20-runtime-manifest.json": RUN_DIR / "runtime-manifest.json",
    }
    for dest_name, src in mapping.items():
        ...
        if src.is_file():
            ...
            write_json(
                dest if dest.suffix == ".json" else BUNDLE / (dest_name + ".missing.json"),
                {"missing": True, "expected": str(src)},
            )
    if diag_root.is_file():
        (BUNDLE / "04b-root-cause.json").write_bytes(diag_root.read_bytes())

    guide = f"""# HBYS Blind E2E Review Guide — RUN_2B

Run type: POST_FIX_FULL_E2E
Generated: {utcnow()}
Project: {project_id}
Document: {doc_id}
Parser pages expected: {EXPECTED_PAGES}

## How to review
1. Read `01-source-hbys-technical-specification.pdf` independently.
2. Compare obligations against `07-requirements.json`.
3. Check `08-compliance-matrix.json` for unjustified decisions.
4. Inspect missing-evidence / risks / conflicts / ambiguities / hidden costs.
5. Cross-check `19-final-report.pdf` counts vs JSON.
6. Parser diagnostic: `04-parser-diagnostic-summary.json` + `05-page-capabilities.json`.

## Frozen rule
Requirement/compliance/risk prompts and company evidence were not changed for RUN_2B.
Only parser routing/batching/checkpoints/timeout layers were allowed to change.
INDEPENDENT_CONTENT_QUALITY = PENDING_EXTERNAL_REVIEW
"""
    (BUNDLE / "21-review-guide.md").write_text(guide, encoding="utf-8")
    ...
    run1_summary = ROOT / "run-1" / "run-summary.json"
    run1 = json.loads(run1_summary.read_text()) if run1_summary.is_file() else {}
    write_json(RUN_DIR / "run1-vs-run2.json", {
        "sameSourceHash": (run1.get("source") or {}).get("sha256") == pdf_sha,
        "samePageCountExpected": EXPECTED_PAGES,
        "sameCompanySnapshotOrg": org_id == run1.get("organizationId"),
        "run1": {
            "pipelineE2EStatus": run1.get("pipelineE2EStatus"),
            "clauses": run1.get("clauseCount"),
            "requirements": run1.get("automaticRequirementCount"),
            "blockers": run1.get("blockers"),
            "projectId": run1.get("projectId"),
        },
        "run2b": {
            "parserTerminal": ready.get("status"),
            "pageCount": page_count,
            "layoutBlockCount": layout_count,
            "pipelineE2EStatus": SUMMARY.get("pipelineE2EStatus"),
            "clauses": SUMMARY.get("clauseCount"),
            "requirements": SUMMARY.get("automaticRequirementCount"),
            "projectId": project_id,
            "parserJobId": parser_job_id,
            "timingsSeconds": TIMINGS,
        },
        "codeConfigDifferences": [
            "bounded checkpointed parsing",
            "selective OCR routing",
            "layered parser timeouts",
            "document consumer in-process poll",
        ],
    })
    (BUNDLE / "run1-vs-run2.json").write_bytes((RUN_DIR / "run1-vs-run2.json").read_bytes())
    ...
    log(f"RUN2B_DONE pipeline={SUMMARY.get('pipelineE2EStatus')} blockers={SUMMARY.get('blockers')}")
